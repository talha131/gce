...
- **ASSIGNMENT_COLUMN**: Which assignment column to process ("D", "E", "F", etc.)
- **SEND_EMAILS**: `true` to send emails, `false` for test mode
...
- **Row 1**: Headers - assignment names must match Gmail draft subjects exactly
...
2. Use placeholders (case-insensitive):
   - `{{Name}}` - Student's name
   - `{{Seat}}` or `{{SeatNumber}}` - Seat number
3. **Subject must exactly match assignment column header (row 1)**
4. Save as draft
...
1. Set `ASSIGNMENT_COLUMN` in Config sheet to column you want to process
2. Ensure column header (row 1) matches Gmail draft subject
...
**Draft not found**: Gmail draft subject must exactly match column header in row 1
